...
<div class="container">
			...
			<table class="table table-bordered">
				<thead>
					<tr>
						...
						<th>
							<c:out value="${aptEmp.apartment.id}" />
						</th>
					</tr>
				</thead>
				<tbody>
					<tr class="success">
						...
						<td>
							<c:out value="${aptEmp.apartment.name}" />
						</td>
					</tr>
					<tr class="success">
						<td>
						...
						<td>
							<c:out value="${aptEmp.apartment.address}" />
						</td>
					</tr>
					<tr class="success">
						<td>
	...
	<div class="row clearfix">
		...
		<div class="col-md-6 column">
			<form class="form-horizontal" role="form" id="aptEmpRotaForm" action="ApartmentEmpRota.do" method="post">
				<div class="form-group">
					...
					<div class="col-sm-7">
						...
						<input class="form-control" id="aptEmpId" name="aptEmpId" type="hidden" value="${aptEmp.id}"/>
					</div>
				</div>
				<div class="form-group">
				...
				<div class="form-group">
					<div class="col-sm-offset-3 col-sm-10">
						 ...
						 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						 <button class="btn btn-success btn-lg" type="button" onclick="window.open('ApartmentEmpRotaAdd.do?aptEmpId=${aptEmp.id}')">添加记录</button>
					</div>
				</div>
			</form>
			<c:if test="${msg == '0'}">
	...
	<div class="row clearfix">
		...
		<div class="col-md-10 column">
			...
			<table class="table table-bordered">
				...
				<c:forEach var="rota" items="${rotas}">
					<tr class="success">
						...
						<td>
							<c:if test="${rota.createTime > 0}">
								<script>timestampToDateStr(<c:out value="${rota.createTime}" />)</script>
							</c:if>
						</td>
						<td>
							<button type="button" class="btn btn-success btn-xs" onclick="modifyRotaSubmit('RotaModify.do?rotaId=${rota.id}')">查看修改</button>
							<button type="button" class="btn btn-success btn-xs" onclick="delRotaSubmit('RotaDel.do?type=0&rotaId=${rota.id}','${rota.id}')">删除记录</button>
						</td>
					</tr>
					</c:forEach>	
				</tbody>
			...
			<c:if test="${page != null}" >
			<form class="form-horizontal" role="form" id="aptEmpRotaPageForm" action="ApartmentEmpRota.do" method="post" >
				<div class="form-group">
					<div class="col-sm-7">
						<input class="form-control" id="aptEmpId" name="aptEmpId" type="hidden" value="${aptEmp.id}"/>
			...
			<div class="col-md-offset-9 col-md-4">
				<ul class="pagination">
					...
					<c:if test="${page > pageLength}">
						<li><a href="javascript:changePage('aptEmpRotaPageForm','page',<c:out value="${pageStart-1}"/>)">&laquo;</a></li>
					</c:if>
					...
					<c:forEach var="linkPage" begin="${pageStart}" end="${pageEnd}" step="1">
						<c:if test="${linkPage == page}">
							<li class="active"><a href="javascript:changePage('aptEmpRotaPageForm','page',<c:out value="${linkPage}"/>);"><c:out value="${linkPage}"/></a></li>
						</c:if>
						<c:if test="${linkPage != page}">
							<li><a href="javascript:changePage('aptEmpRotaPageForm','page',<c:out value="${linkPage}"/>)"><c:out value="${linkPage}"/></a></li>
						</c:if>
					</c:forEach>
					<c:if test="${pageEnd < pageCount}">
						<li><a href="javascript:changePage('aptEmpRotaPageForm','page',<c:out value="${pageStart + pageLength}"/>)">&raquo;</a></li>
					</c:if>
				</ul>
			</div>
			</c:if>
