...
<div class="container">
	...
	<div class="row clearfix">
		...
		<div class="col-md-6 column">
			<form class="form-horizontal" role="form" id="aptEmpForm" action="ApartmentEmp.do" method="post">
				<div class="form-group">
					<label class="col-sm-3 control-label" for="inputState">员工状态</label>
					<div class="col-sm-6">
				...
				<div class="form-group">
					<div class="col-sm-offset-3 col-sm-10">
						 ...
						 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						 <button class="btn btn-success btn-lg" type="button" onclick="window.open('ApartmentEmpAdd.do?aptId=${apt.id}')">添加员工</button>
					</div>
				</div>
			</form>
			<c:if test="${msg == '0'}">
	...
	<div class="row clearfix">
		...
		<div class="col-md-10 column">
			...
			<table class="table table-bordered">
				...
				<tbody>
				<c:forEach var="aptEmp" items="${aptEmps}">
					<tr class="success">
						<td>
							<c:out value="${aptEmp.id}"/>
						</td>
						<td>
							<c:out value="${aptEmp.employee.empId}"/>
						</td>
						<td>
							<c:out value="${aptEmp.employee.name}"/>
						</td>
						<td>
							<c:if test="${aptEmp.employee.sex == 0}">男</c:if>
							<c:if test="${aptEmp.employee.sex == 1}">女</c:if>
						</td>
						<td>
							<c:out value="${aptEmp.employee.phone}" />
						</td>
						<td>
							<c:if test="${aptEmp.employee.state == 0}">在职</c:if>
							<c:if test="${aptEmp.employee.state == 1}">离职</c:if>
						</td>
						<td>
							<c:if test="${aptEmp.createTime > 0}">
								<script>timestampToDateStr(<c:out value="${aptEmp.createTime}" />)</script>
							</c:if>
						</td>
						<td>
							<button type="button" class="btn btn-success btn-xs" onclick="rotaAptEmpSubmit('ApartmentEmpRota.do?aptId=${apt.id}&aptEmpId=${aptEmp.id}')">值班记录</button>
							<button type="button" class="btn btn-success btn-xs" onclick="delAptEmpSubmit('ApartmentEmpDel.do?aptId=${apt.id}&aptEmpId=${aptEmp.id}','${aptEmp.id}')">删除记录</button>
						</td>
					</tr>
					</c:forEach>	
				</tbody>
			...
			<c:if test="${page != null}" >
			<form class="form-horizontal" role="form" id="aptEmpPageForm" action="ApartmentEmp.do" method="post" >
				<div class="form-group">
					<div class="col-sm-7">
			...
			<div class="col-md-offset-9 col-md-4">
				<ul class="pagination">
					...
					<c:if test="${page > pageLength}">
						<li><a href="javascript:changePage('aptEmpPageForm','page',<c:out value="${pageStart-1}"/>)">&laquo;</a></li>
					</c:if>
					...
					<c:forEach var="linkPage" begin="${pageStart}" end="${pageEnd}" step="1">
						<c:if test="${linkPage == page}">
							<li class="active"><a href="javascript:changePage('aptEmpPageForm','page',<c:out value="${linkPage}"/>);"><c:out value="${linkPage}"/></a></li>
						</c:if>
						<c:if test="${linkPage != page}">
							<li><a href="javascript:changePage('aptEmpPageForm','page',<c:out value="${linkPage}"/>)"><c:out value="${linkPage}"/></a></li>
						</c:if>
					</c:forEach>
					<c:if test="${pageEnd < pageCount}">
						<li><a href="javascript:changePage('aptEmpPageForm','page',<c:out value="${pageStart + pageLength}"/>)">&raquo;</a></li>
					</c:if>
				</ul>
			</div>
			</c:if>
      		<c:if test="${aptEmps != null && fn:length(aptEmps) == 0}">
