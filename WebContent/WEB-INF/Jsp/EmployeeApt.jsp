...
<div class="container">
	...
	<div class="row clearfix">
		...
		<div class="col-md-6 column">
			<form class="form-horizontal" role="form" id="empAptForm" action="EmployeeApt.do" method="post">
				...
				<div class="form-group">
					<div class="col-sm-offset-3 col-sm-10">
						 ...
						 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						 <button class="btn btn-success btn-lg" type="button" onclick="window.open('EmployeeAptAdd.do?empId=${emp.empId}')">添加记录</button>
					</div>
				</div>
			</form>
			<c:if test="${msg == '0'}">
	...
	<div class="row clearfix">
		...
		<div class="col-md-10 column">
			<label for="TableName">公寓列表</label><br>
			<c:if test="${page != null}" >
			...
			<table class="table table-bordered">
				...
				<c:forEach var="aptEmp" items="${aptEmps}">
					<tr class="success">
						...
						<td>
							<button type="button" class="btn btn-success btn-xs" onclick="rotaEmpAptSubmit('EmployeeAptRota.do?empId=${emp.empId}&aptEmpId=${aptEmp.id}')">值班记录</button>
							<button type="button" class="btn btn-success btn-xs" onclick="delEmpAptSubmit('EmployeeAptDel.do?empId=${emp.empId}&aptEmpId=${aptEmp.id}','${aptEmp.id}')">删除记录</button>
						</td>
					</tr>
					</c:forEach>	
				</tbody>
			...
			<c:if test="${page != null}" >
			<form class="form-horizontal" role="form" id="empAptPageForm" action="EmployeeApt.do" method="post" >
				<div class="form-group">
					<div class="col-sm-7">
						<input class="form-control" id="empId" name="empId" type="hidden" value="${emp.empId}"/>
			...
			<div class="col-md-offset-9 col-md-4">
				<ul class="pagination">
					...
					<c:if test="${page > pageLength}">
						<li><a href="javascript:changePage('empAptPageForm','page',<c:out value="${pageStart-1}"/>)">&laquo;</a></li>
					</c:if>
					...
					<c:forEach var="linkPage" begin="${pageStart}" end="${pageEnd}" step="1">
						<c:if test="${linkPage == page}">
							<li class="active"><a href="javascript:changePage('empAptPageForm','page',<c:out value="${linkPage}"/>);"><c:out value="${linkPage}"/></a></li>
						</c:if>
						<c:if test="${linkPage != page}">
							<li><a href="javascript:changePage('empAptPageForm','page',<c:out value="${linkPage}"/>)"><c:out value="${linkPage}"/></a></li>
						</c:if>
					</c:forEach>
					<c:if test="${pageEnd < pageCount}">
						<li><a href="javascript:changePage('empAptPageForm','page',<c:out value="${pageStart + pageLength}"/>)">&raquo;</a></li>
					</c:if>
				</ul>
			</div>
			</c:if>
